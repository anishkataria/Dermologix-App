...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    let benignText = "Benign Moles"
    let ringwormText = "Ringworm"
    ...
    @IBAction func photoButton(_ sender: Any) {
        if self.textLabel.text == self.benignText {
            self.performSegue(withIdentifier: "ScanToBenign", sender: nil)
            }
        else if self.textLabel.text == self.measlesText {
            self.performSegue(withIdentifier: "ScanToMeasles", sender: nil)
            }
        else if self.textLabel.text == self.ringwormText {
            self.performSegue(withIdentifier: "ScanToRingworm", sender: nil)
            }
